...
public class TokenService
{
    ...
    public LoginUser getLoginUser(HttpServletRequest request)
    {
        ...
        if (StringUtils.isNotEmpty(token))
        {
            try
            {
                ...
                // 解析对应的权限以及用户信息
                String uuid = (String) claims.get(Constants.TOKEN_CLAIMS_KEY);
    ...
    public String createToken(LoginUser loginUser)
    {
        ...
        claims.put(Constants.TOKEN_CLAIMS_KEY, token);
    ...
    private String getToken(HttpServletRequest request)
    {
        ...
        if (StringUtils.isNotEmpty(token) && token.startsWith(Constants.TOKEN_REQUEST_PREFIX))
        {
            token = token.replace(Constants.TOKEN_REQUEST_PREFIX, "");
        }
        return token;
    }
